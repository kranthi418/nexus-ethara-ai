...
router.get('/stats', async (req, res, next) => {
  try {
    ...
    let projectIds;
    if (role==='admin') {
      const rows = await db.all('SELECT id FROM projects');
      projectIds = rows.map(r=>r.id);
    } else {
      const rows = await db.all('SELECT DISTINCT p.id FROM projects p LEFT JOIN project_members pm ON p.id=pm.project_id WHERE p.owner_id=? OR pm.user_id=?', uid, uid);
      projectIds = rows.map(r=>r.id);
    }
    ...
    for (const pid of projectIds) {
      ...
      tasks.push(...pt);
      const p = await db.get('SELECT status FROM projects WHERE id=?', pid);
    ...
    res.json({ success:true, data:{
      ...
      total_members: ((await db.get('SELECT COUNT(*) as c FROM users'))||{c:0}).c,
    }});
  } catch(err) { next(err); }
});
